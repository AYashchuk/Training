...
public class Test extends MainTest{
	...
	/** array wich using testing method for sorting	 */
	private int [] currentArray;
	/** this name of faster method in current test   */
	private String nameFasterSortMethod;
	/** time of sorting faster method  */
		...
		for(int i=0;i<5;i++){                                                                  // because we have 5 type of sort using cycle
			switch (i){
				case 0:{                                                                       // test bubble sort
					if((currTime = testingSort("Bubble", sorter.createSort(TypeOfSort.BUBBLE),i+1)) < sortTime){ // calling testingSort();
					...
					break;}
				case 1:{
					if((currTime = testingSort("Exchange", sorter.createSort(TypeOfSort.EXCHANGE),i+1)) < sortTime){
					...
					break;}
				case 2:{
					if((currTime = testingSort("Arrays.sort()", sorter.createSort(TypeOfSort.ARRAYS),i+1)) < sortTime){
					...
					break;}
				case 3:{
					if((currTime = testingSort("Merge sort", sorter.createSort(TypeOfSort.MERGE),i+1)) < sortTime){
					...
					break;}
				case 4:{
					if((currTime = testingSort("Recursive sort", sorter.createSort(TypeOfSort.RECURSIVE),i+1)) < sortTime){
